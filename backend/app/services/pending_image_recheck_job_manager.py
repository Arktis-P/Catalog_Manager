...
class PendingImageRecheckJobManager:
    ...
    @classmethod
    def _eligible_query(cls, db: Session, *, after_image_id: int | None = None):
        query = (
    ...
    def _missing_files_count(self, db: Session) -> int:
        missing = 0
        cursor: int | None = None
        while True:
            rows = (
                self._eligible_query(db, after_image_id=cursor)
    ...
    def preview(self, db: Session, *, batch_size: int = DEFAULT_RECHECK_BATCH_SIZE) -> PendingImageRecheckPreview:
        clamped = self._clamp_batch_size(batch_size)
        query = self._eligible_query(db)
        total = query.count()
        first = query.first()
        last = (
            self._eligible_query(db)
        ...
        return PendingImageRecheckPreview(
            ...
            missing_files=self._missing_files_count(db),
            batch_size=clamped,
            first_image_id=first.id if first else None,
            last_image_id=last[0] if last else None,
        )

    def start(self, *, batch_size: int = DEFAULT_RECHECK_BATCH_SIZE) -> PendingImageRecheckJobState | None:
    ...
    def _run(self, job_id: str) -> None:
        ...
        try:
            with job_write_context(job_id):
                batch_size = self.get_job(job_id).batch_size if self.get_job(job_id) else DEFAULT_RECHECK_BATCH_SIZE
                ...
                total = self._eligible_query(db).count()
                self._update(
                    job_id,
                    status="running",
                    phase="rechecking",
                    total=total,
                    message=f"Rechecking identity for {total} pending-review images.",
                )
                settings_service = SettingsService(db)
                hf_token = settings_service.get_hf_token() or None
                hf_wd_model = settings_service.get_hf_wd_model() or None
                cursor: int | None = None

                while not self._is_cancelled(job_id):
                    batch = self._eligible_query(db, after_image_id=cursor).limit(batch_size).all()
                    if not batch:
                        break
                    for image in batch:
