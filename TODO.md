...
- [ ] Danbooru API 연동 (pybooru)
- [ ] 시리즈 위키 페이지 수집
- [ ] `list_of_*_characters` 페이지 자동 탐색
- [ ] wiki / list / post / related tags 기반 character 후보 수집
- [ ] character tag category 필터 및 alias/deprecated 처리
- [ ] character_tag + series_tag 기준 중복 제거
- [ ] 외형 태그 추출 (hair_color, multi_color_hair, hair_shape, eye_color, feature_tags)
- [ ] Series 화면에 "Collect Characters" 작업 버튼 추가
- [ ] 수집 진행 상태 표시 (collecting / collected)
- [ ] Catalog에 수집된 캐릭터 반영
...
- [ ] Settings 화면 (또는 settings 테이블 UI)
...
- [ ] 앱 창 전용 실행 (`launch_app.bat` — Edge/Chrome app mode)
- [ ] production 빌드: frontend dist + backend 단일 포트 서빙
- [ ] (선택) Electron / Tauri 네이티브 래퍼
- [ ] 앱 아이콘 / 창 제목 / 최소 크기 설정
- [ ] 백엔드 subprocess 자동 기동 및 종료 처리
