...
- [x] 시리즈 위키 / `list_of_*_characters` 기반 캐릭터 수집 (Danbooru wiki API)
- [x] 하위 시리즈 위키 탐색 → DB 시리즈 매칭 → 수집 → 상위 병합
- [x] 위키 실패 시 기존 post/패턴 수집으로 fallback
- [x] related tags copyright 기반 시리즈 소속 검증 (`needs_check`)
- [ ] character tag alias/deprecated 처리
