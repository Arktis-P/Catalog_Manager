...
- 상단 카드: 최소 포스트 수 입력 + "전체 캐릭터 목록 수집" 버튼, "선택 캐릭터 통합 태그 수집" / "실패·부분완료 재시도" 버튼, 실행 중인 작업의 진행률/일시정지/재개/취소 컨트롤
- 목록 테이블: 체크박스, 캐릭터명, 포스트 수, 통합 수집 상태 배지, 대표 시리즈, 관련 시리즈 수, 마지막 수집 시각
- 행 클릭(캐릭터명) 시 상세 모달: 외형/성별/시리즈 각각의 세부 상태와 값, 관련 시리즈 전체 목록, 오류 메시지, 재시도 횟수
...
- `frontend/src/pages/CharactersPage.tsx` — 메인 화면
