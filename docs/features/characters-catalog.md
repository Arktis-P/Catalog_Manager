# Characters 탭 (전체 캐릭터 카탈로그)

기존 `Series → Characters` 흐름(시리즈를 먼저 등록하고 그 안에서 캐릭터를 찾는 방식)과 별개로,
Danbooru의 `character` 카테고리 태그 전체를 포스트 수 기준으로 직접 수집하고 관리하는 기능이다.
Series 탭과 관련 데이터/로직은 전혀 변경하지 않았고, 새 테이블과 새 화면으로만 구성했다.

## 무엇을 할 수 있나

- Danbooru character 태그를 포스트 수 내림차순으로 전체 수집 (기본 임계값: 포스트 수 10 이상, UI에서 조정 가능)
- 캐릭터별로 related tags를 **한 번만 조회**해서 외형(머리색/형태/눈색/특징), 성별, 관련 시리즈(copyright tag)를 동시에 분류·저장
- 캐릭터 ↔ 시리즈 다대다 관계 지원 (대표 시리즈 1개 + 관련 시리즈 목록)
- 목록에서 검색(이름/외형), 성별·수집 상태·이미지 생성 여부·커버 이미지 선택 여부 필터, 정렬, 페이지네이션
- 개별/일괄 선택 후 통합 태그 수집, 실패·부분완료 항목만 재시도
- 목록 수집은 중단 지점(체크포인트)을 저장해 재실행 시 이어서 진행 (중복 생성 없이 upsert)
- 통합 태그 수집 작업은 일시정지/재개/취소 가능, 진행 중 캐릭터명과 성공/부분완료/실패 카운트 표시
- 통합 태그 수집은 **여러 캐릭터를 동시에** 처리할 수 있다. 개별/선택/재시도 요청은 이미 다른 수집이
  진행 중이어도 항상 즉시 접수되어 큐에 추가되고, 설정 탭의 "Danbooru 동시 요청 개수"
  (`danbooru_collect_max_concurrent`, Series 수집과 공유하는 설정)만큼 병렬로 실행된다
- 체크박스로 대량 선택(예: 200개)해서 "선택 캐릭터 통합 태그 수집"을 눌러도 **하나의 작업(진행률)**으로
  묶여서 좌측에 카드 1개만 표시되지만, 내부적으로는 캐릭터 단위로 동시 처리 개수(설정값)만큼씩
  병렬로 처리된다 (예: 200개를 올려도 5개씩 동시에 진행되며 진행률은 6/200, 7/200처럼 하나로 갱신됨)
- 통합 태그 조회가 실패해도 이전에 이미 성공(`completed`)한 외형/성별/시리즈 하위 상태는 덮어쓰지 않고
  그대로 보존한다 (예: 머리색은 이미 수집됐는데 이번 재시도에서 API 호출 자체가 실패한 경우, 통합 상태는
  `partial`로 표시되고 머리색 관련 배지는 계속 초록색 유지)
- "미수집 전체 태그 수집" 버튼: 현재 목록에 있는 캐릭터 전체 중 `collect_status`가 `completed`(이미 전부
  수집됨)인 캐릭터는 건너뛰고, 나머지(미수집/실패/부분완료/검토필요)를 `post_count desc, id asc` 순으로
  모아 **5000개씩(기본값, 설정 가능) 하나의 job으로 나눠** 좌측 작업 내역에 여러 개의 진행 카드로 제출한다.
  화면에 표시된 일부 페이지가 아니라 DB 전체를 대상으로 하며, `POST /character-catalog/tags/collect-all`은
  생성된 job 목록(`CatalogJobListResponse`)을 반환한다
- 진행 중인 통합 태그 수집 job은 진행률 바 아래에 **현재 동시에 처리 중인 캐릭터 목록을 세로로 나열**해서
  보여준다 (설정값만큼, 예: 5개면 최대 5줄). 개별 클릭/선택 묶음/재시도/미수집 전체 수집 등 통합 태그 수집을
  시작하는 모든 경로에 공통으로 적용된다
- 각 통합 태그 수집 job은 진행 카드의 ⏸ 버튼으로 일시정지할 수 있다 (이미 실행기에 제출되어 처리 중인
  캐릭터는 끝까지 완료된 뒤 정지), 일시정지 상태에서 × 버튼으로 취소하면 그 시점까지 이미 수집된 결과는
  그대로 DB에 남고 남은 캐릭터만 처리되지 않는다

## 화면 구성

`Series` 탭 오른쪽에 `Characters` 탭이 추가된다 (`frontend/src/pages/CharactersPage.tsx`).

- 상단 카드: 최소 포스트 수 입력 + "전체 캐릭터 목록 수집" 버튼, "선택 캐릭터 통합 태그 수집" / "실패·부분완료 재시도" /
  "미수집 전체 태그 수집" 버튼
- **작업 진행 상태는 페이지가 아니라 좌측(사이드바) 통합 작업 내역(`GlobalTaskBar`)에 표시된다.** Series/Generation 작업과
  동일한 위치에 카드로 나타나며, 진행률 바(`current`/`total` 기준 — 예: 일괄 통합 태그 수집을 시작한 개수를 전체로
  둔 진행률)와 일시정지/재개/취소 버튼을 제공한다. 목록 수집처럼 총 개수를 미리 알 수 없는 작업은 진행률 바가
  불확정(indeterminate) 상태로 표시된다.
- 목록 테이블(한 줄에 캐릭터 하나씩, 축소된 글꼴로 더 많은 정보를 표시). 각 컬럼은 겹치지 않도록 고정 폭을
  지정했다 (`characters-table-compact` 관련 CSS):
  - 체크박스, Danbooru 위키 바로가기(`W`) 버튼
  - Character: 표시명+태그를 같은 줄에, 30자(`30ch`) 고정 폭으로 표시 (넘치면 말줄임 + hover 시 전체 텍스트 title)
  - Post count
  - 통합 상태: 전체 상태 배지 + 외형/성별/시리즈 개별 수집 여부 미니 배지 (고정 폭 190px로 대표 시리즈 컬럼과 겹치지 않음)
  - 대표 시리즈
  - 머리색(관련도 상위 2개) · 멀티컬러 · 머리 모양 · 눈색 · 기타 외형(각각 관련도 1위 1개) — 항목별로 별도 컬럼에 표시,
    미수집 항목은 `-`
  - 성별: 1boy는 하늘색, 1girl은 분홍색, 그 외(no_humans/미수집)는 회색 배지
  - 행별 "태그 수집" 버튼: 해당 캐릭터 하나만 즉시 통합 태그 재수집 (일괄 작업과 별개로 큐잉됨)
- 관련 시리즈 "개수"와 "마지막 수집 시각" 컬럼은 표에서 제거했다 (필요 시 상세 모달에서 확인)
- 행 클릭(표시명) 시 상세 모달: 외형/성별/시리즈 각각의 세부 상태와 전체 값, 관련 시리즈 전체 목록, 마지막 수집
  시각, 오류 메시지, 재시도 횟수

## 데이터 모델

기존 `series`, `characters` 테이블은 변경하지 않았다. 새로 추가한 테이블:

- **`global_characters`** (`backend/app/models/global_character.py`)
  독립적인 캐릭터 엔티티. `character_tag`(unique), `post_count`, 통합 상태(`collect_status`)와 세부 상태
  (`appearance_status`, `gender_status`, `series_status`)를 각각 컬럼으로 분리해서, 일부 분류가 실패해도
  성공한 결과는 보존하고 실패한 부분만 별도로 표시/재시도할 수 있게 했다. 외형 태그(hair/eye/feature),
  성별, 오류 메시지, 재시도 횟수, 마지막 수집 시각을 함께 저장한다.

- **`character_series_links`** (`backend/app/models/character_series_link.py`)
  `global_characters` ↔ `series` 다대다 조인 테이블. `copyright_tag`는 항상 원본 태그 문자열을 보관하고,
  `series_id`는 매핑된 Series row(nullable). `relevance_rank`/`is_primary`로 대표 시리즈를 표시하고,
  `is_user_edited`가 true인 링크는 재수집 시 덮어쓰지 않는다.

시리즈 매핑 실패 처리: related tags에서 발견한 copyright tag가 기존 `series` 테이블에 없으면
자동으로 새 Series row를 생성한다 (`status="pending"`, note에 자동 생성 표시). 기존 Series 수집/외형추출
파이프라인에는 영향을 주지 않는다.

## 수집 로직

- **목록 수집** (`backend/app/integrations/danbooru/character_catalog_collector.py`,
  `backend/app/services/character_catalog_service.py::collect_list`)
  `tags.json?search[category]=4&search[order]=count` 를 페이지 단위로 순회하며 포스트 수 임계값 미만이
  나오면 중단(내림차순 정렬이므로 이후 페이지도 전부 미달). 페이지마다 upsert 커밋하고, 마지막 처리 페이지를
  `settings` 테이블에 체크포인트로 저장해 재실행 시 이어서 진행한다.

- **통합 태그 수집** (`character_catalog_service.py::collect_tags_for_character`)
  `related_tag.json?query=<tag>` 를 **category 필터 없이 한 번** 호출하면 응답에 모든 카테고리의
  관련 태그가 섞여 반환된다(태그별 `category` 필드 포함). 여기서:
  - 외형/성별: 기존 `appearance_extractor.py`의 `extract_appearance_tags` 그대로 재사용
  - 시리즈: 새로 추가한 `extract_copyright_tags` (category == 3인 태그만 빈도순 추출)

  세 분류는 각각 독립적으로 try/except 처리되어, 하나가 실패해도 나머지는 저장된다. 전체 상태는
  외형/시리즈 성공 여부로 `completed`/`partial`/`failed`를 판정한다. related tags 조회 자체가
  실패한 경우를 포함해 모든 실패 처리 지점에서, 이미 `completed` 상태인 하위 항목은 실패로
  덮어쓰지 않고 유지한다 (재시도 시 기존에 성공한 데이터 보존).

## 백엔드 API (`/api/character-catalog/...`)

| Method | Path | 설명 |
|---|---|---|
| GET | `/character-catalog/characters` | 목록 조회 (search, gender, collect_status, series_id, post_count 범위, 정렬, 페이지네이션) |
| GET | `/character-catalog/characters/{id}` | 상세 조회 (관련 시리즈 전체 포함) |
| POST | `/character-catalog/list/start` | 전체 목록 수집 시작 (`min_post_count`, `restart`) |
| POST | `/character-catalog/tags/start` | 선택 캐릭터 통합 태그 수집 시작 (`character_ids`) |
| POST | `/character-catalog/tags/retry-failed` | 실패/부분완료 캐릭터만 재시도 |
| GET/POST | `/character-catalog/jobs...` | 작업 조회/일시정지/재개/취소 (Series 작업 큐와 완전히 분리된 별도 큐) |

작업 관리는 `backend/app/services/character_catalog_job_manager.py`에서 처리하며, 기존
`SeriesJobManager`(`collect_job_manager.py`)와 완전히 독립된 큐/스레드를 사용한다. DB 커밋 직렬화만
기존 `db_write_queue`를 공유한다.

job(개별 클릭/선택 묶음/재시도 묶음 단위)은 접수되는 즉시 각자의 스레드로 시작된다. 실제 동시성 제한은
job 레벨이 아니라 **캐릭터 단위**로 걸려있다: 모든 통합 태그 수집 job이 공유하는 하나의
`ThreadPoolExecutor`(`_get_tags_executor`, 설정 탭의 "Danbooru 동시 요청 개수" =
`SettingsService.get_collect_max_concurrent` 만큼 워커 보유)에 캐릭터 하나당 작업 1개씩 제출한다.
그 결과:
- 캐릭터 200개를 한 번에 선택해 시작해도 실제로는 설정값(예: 5개)만큼만 동시에 Danbooru를 호출하고,
  나머지는 실행기 내부 큐에서 대기하다가 순서대로 처리된다.
- 여러 번의 개별/선택 요청이 동시에 존재해도 시스템 전체에서 동시에 진행 중인 캐릭터 처리 개수는
  항상 설정값 이하로 유지된다 (job이 여러 개여도 총 동시 처리량은 공유 실행기 하나로 제한됨).
- 하나의 선택 묶음(batch)은 여전히 진행률 카드 1개로 표시되며, 그 안에서 완료된 캐릭터 수만큼
  `current`/`success_count`/`partial_count`/`failed_count`가 누적 갱신된다.

## 프론트엔드 구성

- `frontend/src/context/CharacterCatalogJobContext.tsx` — 기존 `CollectJobContext`와 별도의 폴링
  컨텍스트 (Series 작업 알림/상태에 영향 없음)
- `frontend/src/pages/CharactersPage.tsx` — 메인 화면 (작업 버튼만 있고, 진행 상태 UI는 없음)
- `frontend/src/components/CatalogProgressPanel.tsx` — 좌측 통합 작업 내역에 표시되는 진행 카드
  (진행률 바 포함). `frontend/src/components/GlobalTaskBar.tsx`가 Series/Generation 작업과 함께
  이 컴포넌트를 렌더링한다.
- `frontend/src/utils/characterCatalogStatus.ts` — 상태 배지/라벨 매핑
- 타입: `GlobalCharacter`, `CharacterSeriesLinkInfo`, `CatalogJob` (`frontend/src/types/index.ts`)
- API 클라이언트: `frontend/src/api/client.ts`의 `listGlobalCharacters`, `startCatalogListJob`,
  `startCatalogTagsJob`, `retryFailedCatalogTags`, `listCatalogJobs` 등

## 캐릭터 목록 중심 이미지 생성 · 리뷰

기존 시리즈 중심 Generation/Review 기능(`Character`/`Image`/`GenerationJob`/`Review`, series 상태 관리 포함)과
**완전히 독립적으로**, `GlobalCharacter`(캐릭터 카탈로그) 목록을 대상으로 바로 이미지를 생성·검수할 수 있는
경로를 추가했다. 시리즈 소속 여부와 무관하게 특징 태그만 수집되면 바로 생성 가능하다.

### 데이터 모델

series 전용 테이블(`characters`, `images`, `generation_jobs`, `reviews`)은 전혀 건드리지 않고, 동일한 구조를
그대로 미러링한 새 테이블만 추가했다 (SQLite에서 기존 NOT NULL 컬럼을 nullable로 바꾸는 위험한 마이그레이션을
피하기 위한 선택):

- `global_character_images` (`backend/app/models/global_character_image.py`)
- `global_character_generation_jobs` (`backend/app/models/global_character_generation_job.py`)
- `global_character_reviews` (`backend/app/models/global_character_review.py`)

`GlobalCharacter`에 `images`/`generation_jobs`/`review` 관계를 추가했다.

### 이미지 생성

- **캐릭터 목록 페이지**(`CharactersPage.tsx`): 체크박스 선택 후 "선택 캐릭터 이미지 생성" 버튼, 또는
  "현재 페이지 이미지 생성" 버튼(현재 페이지에 표시된 캐릭터 전체를 체크 → 생성 시작 → 체크 해제), 행별
  "이미지 생성" 버튼(특징 태그 수집 완료 캐릭터만 활성화)이 있다.
- **Generation 페이지**(`GenerationPage.tsx`): 상단에 "시리즈" / "캐릭터 목록" 탭이 생겼다. "캐릭터 목록" 탭은
  시리즈 선택 대신 특징 태그 수집 완료 + 아직 이미지가 생성되지 않은 `GlobalCharacter` 목록을 보여주고,
  체크박스로 최대 300개까지 선택해 프롬프트 레벨을 지정한 뒤 시작할 수 있다.
- **큐잉**: 캐릭터 목록 생성 job은 한 번에 하나만 실행되며, 이미 진행 중일 때 새로 시작하면 좌측 작업
  목록에 대기(`queued`) 상태로 올라가 이전 job이 끝나는 즉시 자동으로 이어서 시작된다
  (`GenerationJobManager._character_queue`, series별 동시 실행과는 별개의 큐).
- 백엔드: `GET /generation/characters/candidates`, `POST /generation/characters/start`. 시작된 job은
  series 기반 생성과 **동일한 job 목록/엔드포인트**(`/generation/jobs...`, `GenerationJobContext`,
  `GlobalTaskBar`)에 `series_id=0`, `series_tag="캐릭터 N명"`으로 표시되므로 일시정지/재개/취소, 진행률 표시가
  전부 기존 UI 그대로 재사용된다.
- 프롬프트/와일드카드 생성 로직(`generation_prompt_builder.build_character_core`,
  `wildcard_writer.write_character_wildcard`)은 series용과 동일한 함수를 그대로 재사용한다
  (`GlobalCharacter`가 `Character`와 같은 외형 필드 이름을 가지도록 설계되어 있어 가능).

#### 프롬프트/와일드카드 생성 규칙

캐릭터 1명은 `build_character_core()`가 `{{이름, [[태그들]]}}` 형태로 압축한다. 프롬프트
레벨(1~5)에 따라 포함되는 외형 정보가 늘어난다 (예: `hatsune_miku`,
`hair_color=aqua_hair`, `hair_shape=twintails, long_hair`, `eye_color=aqua_eyes`,
`feature_tags=hair_ornament, headphones`):

| 레벨 | 포함 정보 | 결과 예시 |
|---|---|---|
| 1 | 캐시된 `generation_prompt`가 있으면 그대로, 없으면 레벨2와 동일 (GlobalCharacter는 캐시 필드가 없어 항상 즉석 계산) | `{{hatsune miku, [[aqua hair]]}}` |
| 2 | 머리색만 | `{{hatsune miku, [[aqua hair]]}}` |
| 3 | + 머리 모양, 눈 색 | `{{hatsune miku, [[aqua hair, twintails, long hair, aqua eyes]]}}` |
| 4~5 | + 특징 태그(feature_tags) | `{{hatsune miku, [[aqua hair, twintails, long hair, aqua eyes, hair ornament, headphones]]}}` |

밑줄(`_`)은 공백으로 치환되고, `multi_color_hair`가 있으면 `streaked_hair`를 제외한 태그가
머리색 뒤에 추가된다.

전체 NAIA 프롬프트는 `build_prompt_from_character_core()`가
`{prefix},\n\n{character_core 또는 와일드카드 토큰},\n\n{suffix}` 형태로 조립한다.
prefix/suffix/negative는 `input/prompt_templates/generation_prefix.txt` ·
`generation_suffix.txt` · `negative_prompt.txt` (없으면 코드 기본값)에서 읽고,
`{gender}`→`1girl`/`1boy`/`no_humans`, `{portrait}`→`portrait`로 치환한다. 예 (레벨 3,
여성 캐릭터):

```
__set_artists_4.5.2__, __set_qualityTags__,

1girl,,

{{hatsune miku, [[aqua hair, twintails, long hair, aqua eyes]]}},

solo, solo focus, portrait, smile
```

체크박스로 여러 캐릭터를 한 번에 생성하면 캐릭터 코어를 프롬프트에 직접 넣지 않고
**와일드카드 파일**(`{NAIA}/user-data/wildcards/catalogue_manager/{queue_id 슬러그}_characters.txt`)을
만들어 캐릭터당 한 줄씩 기록한다:

```
{{hatsune miku, [[aqua hair, twintails, long hair, aqua eyes]]}}
{{kagamine rin, [[blonde hair, short hair, orange eyes]]}}
{{megurine luka, [[pink hair, long hair, aqua eyes]]}}
```

이때 실제 프롬프트의 캐릭터 코어 자리에는 `__*catalogue_manager/{queue_id 슬러그}_characters__`
와일드카드 토큰이 들어가 NAIA가 매 이미지마다 한 줄을 랜덤으로 뽑아 쓴다. 시리즈 생성과
캐릭터 목록 생성 모두 이 로직(`build_character_core`, `write_character_wildcard`)을
동일하게 재사용한다.

### 리뷰

`Review` 페이지의 "Catalog Review" 모드 안에 "시리즈" / "캐릭터 목록" 내부 탭을 추가했다
(`frontend/src/components/review/GlobalCatalogReviewPanel.tsx`). 캐릭터 목록 탭은 시리즈 선택 없이
`GET/POST /review/catalog-global...` 엔드포인트로 이미지 표시·선택(`CatalogReviewRow` 컴포넌트를 그대로
재사용), 태그 on/off, 프롬프트 수정, 평점, 완료/되돌리기 기능을 제공한다. 가상 스크롤·키보드 단축키·시리즈
이동·NAIA 재생성 등 시리즈 모드의 고급 기능은 1차 구현 범위에서 제외했다 (필요 시 후속 작업으로 확장 가능).

## 알려진 제약 / 후속 과제

- Danbooru `tags.json`의 `order=count` 페이지네이션은 매우 깊은 페이지(수십만 건)에서 성능이 떨어질 수
  있다. 대량 수집은 임계값을 단계적으로 낮춰가며(예: 500 → 100 → 10) 실행하는 것을 권장한다.
- 대표 시리즈 수동 지정, 관련도 수동 조정, 시리즈 별칭/병합, 자동 갱신 스케줄, 고급 복합 필터는 1차
  구현 범위에서 제외했다 (요청서 11절 참고).
