...
export const api = {
  ...
  previewPendingImageRecheck: (batchSize?: number) =>
    request<PendingImageRecheckPreview>(
      `/generation/v2/pending-image-recheck/preview${buildQuery({ batch_size: batchSize })}`,
    ),

  startPendingImageRecheck: (batchSize?: number) =>
    request<PendingImageRecheckJob>("/generation/v2/pending-image-recheck/start", {
      method: "POST",
      body: JSON.stringify({ batch_size: batchSize ?? 200 }),
    }),
